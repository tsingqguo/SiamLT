...
%
%   Luca Bertinetto, Jack Valmadre, Joao Henriques, 2016 modified by Qing
%   Guo
% -------------------------------------------------------------------------------------------------
    % Default parameters (set the experiment-specific ones in run_experiment)
    opts = initTrainParams();
...
% -------------------------------------------------------------------------------------------------
    net = make_HOGConv(opts);
    handcraft_cell = opts.feat_global.cell_size;
    num_channel = opts.feat_global.num_channel;
    % Save the net graph to disk.
    inputs = {'exemplar', [floor((opts.exemplarSize-handcraft_cell)./[4 4])+1 num_channel opts.train.batchSize], ...
              'instance', [floor((opts.instanceSize-handcraft_cell)./[4 4])+1 num_channel opts.train.batchSize]};
...
% -------------------------------------------------------------------------------------------------
    ...
    sizes = net.getVarSizes({'exemplar', [floor((opts.exemplarSize-handcraft_cell)./[4 4])+1 num_channel], ...
                             'instance', [floor((opts.instanceSize-handcraft_cell)./[4 4])+1 num_channel]});
